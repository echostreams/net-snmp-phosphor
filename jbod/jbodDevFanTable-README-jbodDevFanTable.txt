...
------------------------------------------------------------------------
  ...
  the jbodDevFanTable table.  The agent will start by looking for the right
  ...
  file defaults/table-jbodDevFanTable.m2d, and then re-running mib2c.
    ...
    in the jbodDevFanTable.h header.
    ...
    will be generated:
      ...
      jbodDevFanTable_allocate_data
      jbodDevFanTable_release_data
 ...
 Index(es) for the jbodDevFanTable table
  ------------------------------------------------------------
  The index(es) for the jbodDevFanTable table are:
...
************************************************************************
jbodDevFanTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the jbodDevFanTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: jbodDevFanTable_data_access.[c|h]
------------------------------------------------------------------------
  The jbodDevFanTable_data_access file contains the interface to your data in
  ...
  FUNC : jbodDevFanTable_indexes_set
  WHERE: jbodDevFanTable_data_access.c
...
File: jbodDevFanTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : fanDescription_get
...
File: jbodDevFanTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      jbodDevFanTable table.

      Conversely, the examples below may not include some functions that
      were generated for the jbodDevFanTable table.

To watch the flow of the jbodDevFanTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        jbodDevFanTable
        verbose:jbodDevFanTable
        internal:jbodDevFanTable

e.g.
        snmpd -f -Le -DjbodDevFanTable,verbose:jbodDevFanTable,internal:jbodDevFanTable
...
/*---------------------------------------------------------------------
 * INWIN-JBOD-MIB::jbodDevFanEntry.fanIndex
 * fanIndex is subid 1 of jbodDevFanEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.47464.1.2.4.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE fanIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------

...
------------------------------------------------------------------------
  ...
  the jbodDevLogTable table.  The agent will start by looking for the right
  ...
  file defaults/table-jbodDevLogTable.m2d, and then re-running mib2c.
    ...
    in the jbodDevLogTable.h header.
    ...
    will be generated:
      ...
      jbodDevLogTable_allocate_data
      jbodDevLogTable_release_data
 ...
 Index(es) for the jbodDevLogTable table
  ------------------------------------------------------------
  The index(es) for the jbodDevLogTable table are:
...
************************************************************************
jbodDevLogTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the jbodDevLogTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: jbodDevLogTable_data_access.[c|h]
------------------------------------------------------------------------
  The jbodDevLogTable_data_access file contains the interface to your data in
  ...
  FUNC : jbodDevLogTable_indexes_set
  WHERE: jbodDevLogTable_data_access.c
...
File: jbodDevLogTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  ...
  -------------------
  TODO : retrieve column data from raw data
  FUNC : logDescription_get



File: jbodDevLogTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      jbodDevLogTable table.

      Conversely, the examples below may not include some functions that
      were generated for the jbodDevLogTable table.

To watch the flow of the jbodDevLogTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        jbodDevLogTable
        verbose:jbodDevLogTable
        internal:jbodDevLogTable

e.g.
        snmpd -f -Le -DjbodDevLogTable,verbose:jbodDevLogTable,internal:jbodDevLogTable
...
/*---------------------------------------------------------------------
 * INWIN-JBOD-MIB::jbodDevLogEntry.logIndex
 * logIndex is subid 1 of jbodDevLogEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.47464.1.2.6.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE logIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------

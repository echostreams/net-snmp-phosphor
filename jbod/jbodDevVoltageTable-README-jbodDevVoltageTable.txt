...
------------------------------------------------------------------------
  ...
  the jbodDevVoltageTable table.  The agent will start by looking for the right
  ...
  file defaults/table-jbodDevVoltageTable.m2d, and then re-running mib2c.
    ...
    in the jbodDevVoltageTable.h header.
    ...
    will be generated:
      ...
      jbodDevVoltageTable_allocate_data
      jbodDevVoltageTable_release_data
 ...
 Index(es) for the jbodDevVoltageTable table
  ------------------------------------------------------------
  The index(es) for the jbodDevVoltageTable table are:
...
************************************************************************
jbodDevVoltageTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the jbodDevVoltageTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: jbodDevVoltageTable_data_access.[c|h]
------------------------------------------------------------------------
  The jbodDevVoltageTable_data_access file contains the interface to your data in
  ...
  FUNC : jbodDevVoltageTable_indexes_set
  WHERE: jbodDevVoltageTable_data_access.c
...
File: jbodDevVoltageTable_enums.h
------------------------------------------------------------------------
...
File: jbodDevVoltageTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : voltageDescription_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : voltageStatus_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : voltageValue_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : voltageUnit_get



File: jbodDevVoltageTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      jbodDevVoltageTable table.

      Conversely, the examples below may not include some functions that
      were generated for the jbodDevVoltageTable table.

To watch the flow of the jbodDevVoltageTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        jbodDevVoltageTable
        verbose:jbodDevVoltageTable
        internal:jbodDevVoltageTable

e.g.
        snmpd -f -Le -DjbodDevVoltageTable,verbose:jbodDevVoltageTable,internal:jbodDevVoltageTable
...
/*---------------------------------------------------------------------
 * INWIN-JBOD-MIB::jbodDevVoltageEntry.voltageIndex
 * voltageIndex is subid 1 of jbodDevVoltageEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.47464.1.2.3.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE voltageIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------

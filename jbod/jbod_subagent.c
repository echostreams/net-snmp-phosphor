 ...
 */
/*
 You can run the agent as a regular SNMP agent, or as an AgentX sub-agent. 
 The default is to run as a sub-agent. 
 While testing, the -f (do not fork into background) and -L (log to stderr) options are recommended.
 
 AgentX Sub-agent:	    jbod-subagent -f -L
 Standard SNMP Agent:	jbod-subagent -M -f -L [udp[:port]]

 Note that running the sub-agent application as a master agent will run without any access control. 
 All SNMPv1 and SNMPv2c requests will be accepted, regardless of the community string.
*/
...
static void usage(void) {
   printf("usage: jbod-subagent [-D<tokens>] [-f] [-L] [-M] [-H] [LISTENING ADDRESSES]\n"
...
main (int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "D:fHLM-:x:")) != EOF)
    ...
    case 'H':
      ...
      init_agent(AGENT_NAME);        /* register our .conf handlers */
